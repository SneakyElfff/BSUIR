...
def upload(client, message, address):
    ...
    if os.path.exists(filename + '.temp'):
        print(f"The client {address} has continued to upload {filename}.")
...
def download(client, message, address):
    ...
    if os.path.exists(filename):
        with open(filename, "rb") as file:
            ...
            sent_size = int(client.recv(1024).decode('utf-8'))
            if sent_size != 0:
                print("The download continues from client {address}")
                file.seek(sent_size)
            ...
            while True:
                ...
                try:
                    ...
                    print("\nDownloading from client {address} was interrupted.")
                    return "File wasn't downloaded.".encode('utf-8')

        print(f"File '{filename}' sent (client {address})")
...
def handle_client(server_socket, client_socket, address):
    try:
        welcome(client_socket)

        while True:
            data = client_socket.recv(1024)
            print(f"Data received: {data.decode('utf-8')} from client {address}")

            if not data:
                print("\nConnection with client {address} is lost.")
                client_socket.close()
                break
            ...
            if data.startswith(b'ECHO'):
                ...
                response = upload(client_socket, data, address)
                ...
                response = download(client_socket, data, address)
                ...
                print("Closing the connection with client {address}")
                client_socket.close()
                break
...
def main():
    ...
    while True:
        ...
        # child process for handling clients
        client_process = multiprocessing.Process(target=handle_client, args=(server_socket, client_socket, client_address))
